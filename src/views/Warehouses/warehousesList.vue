...
import type { WarehousesListResponse , WarehousesListRow } from '@buf/ahmeddarwish_mln-rms-core.bufbuild_es/rms/v1/products_warehouses_definitions_pb'
...
const { records, deletedRecords, options } = await apiClient.warehousesList({})
const dataKey = "warehouseId"
...
const headers: Record<string, ITableHeader> = {
    'warehouseId': new TableHeaderLink('warehouseId', {
        sortable: true,
        isGlobalFilter: true,
        router: viewRouter
    }),
    'warehouseName': new TableHeaderText('warehouseName', {
        sortable: true,
        isGlobalFilter: true,
        filter: {
    ...
    'branchesCount': new TableHeaderCount('branchesCount', {
        sortable: true,
        filter: {
            ...
            input: {
                ...
                placeholder: t("branchesMoreThan")
            }
        }
    }),
...
const tableProps: DataListProps<WarehousesListResponse, WarehousesListRow> = {
    title: "warehouses",
    dataKey: "warehouseId",
    records: records,
    exportable: true,
    ...
    displayType: "card",
    fetchFn: apiClient.warehousesList as any,
            ...
            <DataList class="sm-column" :displayType="tableProps.displayType" :fetchFn="tableProps.fetchFn"
                ...
                <template #start="{ data }">
                    <div class="flex flex-column justify-content-center align-items-center">
                        <h4>{{ $t('Branches') }} </h4>
                        <span class="text-center font-bold"> {{ data.branchesCount }} </span>
                    </div>
                </template>
                <template #end="{ data }">
